...
public class BlockClusterNonogramMultiThread extends AbstractNonogrammProblem {

	public BlockClusterNonogramMultiThread(int line, int column) {
		super(line, column);
	}
	public BlockClusterNonogramMultiThread(String pathNono, String pathCsv) {
	...
	Runnable initInblockVarRow = new Runnable() {
			...
			for (int i = 0; i < square.length; i++) {
				...
				for (int j = 0; j < square[i].length; j++) {
					...
					if(lineData.get(i).size()>0){
						...
						solver.post(LCF.ifThenElse(square[i][j], LCF.or(createOrConstraintXij1(i,j,square[i][j],rowCluster,lineData)),
								LCF.and(createOrConstraintXij0(i,j,square[i][j],rowCluster,lineData))));
					}
				}
			}
	...
	Runnable initInBlockVarColumn = new Runnable() {
			...
			for (int i = 0; i < transpose.length; i++) {
				...
				for (int j = 0; j < transpose[i].length; j++) {
					...
					if(columnData.get(i).size()>0){
						//si x[i][j]=1 alors .... sinon ...
						solver.post(LCF.ifThenElse(transpose[i][j], LCF.or(createOrConstraintXij1(i,j,transpose[i][j],columnCluster,columnData)),
								LCF.and(createOrConstraintXij0(i,j,transpose[i][j],columnCluster,columnData))));
					}
				}
			}	
...
//		buildCluster(columnCluster,columnData,line,"column");
		...
		initInblockVarRow.run();
		initInBlockVarColumn.run();
...
//		 solver.set(IntStrategyFactory.minDom_LB(ArrayUtils.flatten(square)));
	}
...
//	}
	...
	public static void main(String[] args) {
		new BlockClusterNonogramMultiThread(args[1],args[3]).execute(args);
	}
